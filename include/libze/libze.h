...
typedef struct bootpool_t {
    ...
    char root_path_full[ZFS_MAX_DATASET_NAME_LEN];    /**< Dataset root path with prefix (e.g. "bpool/boot/env/ze-" or "bpool/boot/env/" if no prefix is set) */
    char dataset_prefix[ZFS_MAX_DATASET_NAME_LEN];    /**< Dataset prefix (e.g. "ze" for "ROOT_PATH/ze-ENV")                    */
    char env_activated_path[ZFS_MAX_DATASET_NAME_LEN];/**< Path to the boot dataset of the currently activated boot environment */
    char env_running_path[ZFS_MAX_DATASET_NAME_LEN];  /**< Path to the boot dataset of the currently running boot environment   */
} bootpool_t;
...
libze_error
libze_boot_env_name(libze_handle *lzeh, const char *dataset, size_t buflen, char *buf);
